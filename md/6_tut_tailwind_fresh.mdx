...
### Why:
Fresh doesn't have a buildstep, but Tailwind CSS requires a buildstep to generate the necessary CSS from its utility classes.
...
Assuming you have the Tailwind CSS IntelliSense plugin installed you can already see the Intellisense working if you hover over a Taiwlind class in "index.tsx".
...
Tailwind should now be fully functional. Change or add a Twailwind class in "index.tsx" to see if it works.
...
Install the VS Code Prettier Extention as well as the Prettier node module if you have not done so already in a previous step:
