...
[![Addresses dropdown](https://remblog.s3.af-south-1.amazonaws.com/address_finder.PNG)](https://remblog.s3.af-south-1.amazonaws.com/address_finder.PNG)
...
I would have prevered the route be the other way around, but that can easily be fixed by pressing the "Reverse Order" button.
...
Whats this?:  
There are many types of vehicles that a company might use, this feature alows a user or company to model all their vehicles in their fleet.
To create a vehicle requires some information, like fuel consumption and the class of vehicle.
The class of vehicle correspons with how big it is, and how much toll fees it needs to pay.
Class 1 might be a car and class 4 might be a big long haul truck.

The app calculates fuel costs and toll costs accoridng to the given fual consumption and vehcielc class. 
An example will be shown later.

How does it work?:  
The server creates a vehicle document and places it into an array in the workspace document. This allows vehicles to be shared between all users beloging to a workspace.
You will note that the create vehicle dialog asks for a licence plate and vehicle name. 
...
I figured that displying the licence plate and vehicle name would have been handy information.
...
For example, according to the cost graph person 5 will only be charged R2.53 for his delivey cost (assuming the company does not want to make a profit on deliveries) because
...
This way each delivery's cost would have been calulated in isolation witout additional deliveries (ie start -> deliver to single adddress -> return), 
...
The app was stil in its infancy when I decided to step into the office of a rather big mattress delivery company and gave them a demo.
...
I added a bunch of features specificly per his request, but he sugested these features every time I demoed a new version to him, not out of experience by using the app.

I might be mistaken but I believe it might come down to what Tony Fadel said in his book Build: 
People will continue using what they know even if it is inverior to something new. They will choose familiarity over betterment.
...
Even though the meeting was fantastic and we exhnaged a many ideas, they did not realy have a need for the app in its current state.

She suggested that I talk to their tracking company so it can be integrated with their services. 
She would have likes that the planned route be shared with the tracking company, and when the truck deviates unexpectadly they can respond if needed.
...
I dare say that that I believe that my approach could have been adapted better to sove their problem.

From their perspective tracking comes first (they are a tracking company after all), 
but this creates a problem in that except for tracking they dont create an incentive 
to use their app. Ease of use in terms of route finding and planning was an afterthought.

I think the better approach would have been to create an app that has excelenet route finding and planning ababilties and add tracking to that, not the other way around.
I admit I might be wrong and biased in my observations, but until I can see a hands on demo I will stay by my them.

These were the two noteworthy events, I did a few others, bothched some but I learned a lot, and I quite enjoyed it.

### Moving On

I cant say im sitting on a yhach, drinking champaign and smoking a cuban sigare while writting this.
I was not awarded with financial success but rather self betterment and expantion of my knowledge and skills.
...
Exclucing expensises, the app was suffering from technical debt, I did not settle on a great arhcitecture, 
there was spagetti dependencies everywhere and it was getting hard to change and add features.

The scope has also greatly expanded beyond my abilities and time. It was not a simple route finding app anymore. 
It had a lot of intrecate features and supporting apps, like a mobile app for drivers and the "live tracking" app Venice.
...
For anyone as curious as I am here is what I used and leanred to build the app:
...
The github repo is available here: Maro Polo

If I could change a few things I would not have used MongoDB, used Vite + React if not NextJS and used Tailwind earlier.

I am not against using MongoDB, I think it is a realy good DB but there are a few things I dont like about it.

For React I think NextJS is amazing although I am starting to thing there are other frameworks that are doing things better than it is (not neccesarily React Frameworks).
Fresh for instance, it is realy well though out. Solid is also interesting, especially with Solid Start. Im keeping a close eye on that.
And Lastly Qwik, I think resumability might just be the way forward. (havent explored RSC's at the time of writing).

After trying Tailwind I very much started to like it. It works well and it is extremely flexable.
...
With this project I tried and failed. But I am proud and greatfull of what I achieved and that I had this opertunity.
