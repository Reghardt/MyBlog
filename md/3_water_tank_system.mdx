...
[![The add in](https://remblog.s3.af-south-1.amazonaws.com/3_water_tank_system/first_prototype_crop.jpg)](https://remblog.s3.af-south-1.amazonaws.com/3_water_tank_system/first_prototype_crop.jpg)
...
I decided to go with a wire instead of the wireless NRF transcevers as I found them to be unreliable when there are too many walls in between them, which there were many..

Additionaly my Father told me there is an existing 4 wire cable from an old alarm system that was not being used anymore that stretched from where the control unit was going to be placed, to very near to the water tank.
Very convenient.
...
If there were to be 3 seperate units connected to the same bus, ie a sensor, control and relay unit, it would have been wise to make the control unit the master, this way it could request sensor reading from the sensor unit every X seconds and act upon them by sending the relay unit commands, like turning the pump on or off.

I opted to make the sensor unit the slave in this case as I could not get the request/response model to work in time, but in this case it would not have mattered anyway as a seperate relay unit was not needed
...
There were some erronious readings, although signifficantly less than I expected, and nothing that cant be handled in software.
...
Bottom left is a blank pcb without anything solderd or attached.
...
After that I attached the sensor unit to one end and the control unit to the other. I tested to see if the sensor unit can be powered through this wire in adition to using it to send sensor readings. Surprisingly it worked great.
...
Meanwhile, I climbed up the water tank and found the top to be bulged inward. How this happend I have no idea, but it wasnt good.
...
Here my Grandfather and one of his workers was busy constructing a platform for the new tank. This one was going to be placed on the ground and use a preasure pump. It was conveniently placed right next to the tool shed where the wire exits from.
...
For the control unit I used a dremmel tool to cut out a rectange for the screen to fit in. For the keypad I cut a little slit for the membrane cable.
