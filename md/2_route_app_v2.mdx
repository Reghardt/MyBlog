...
If a valid token does exist they can procede to use the app.


How does it work?:  
Login with google creates a bearer token using Next Auth. 
The bearer contains the user's google ID, 
used to identify the user for subseqent server calls.
...
It then subrtacts the associated amount of tokens from the user's tokens and sends the new amount back to the client.
...
The first and last elements are treated as the start and end addreses.
Elements in between are treated as stopovers.
Each stopover has controlls for swaping itself with the above or below stopover.
A stopover cannot be swaped with a start or stop address.
...
How does it work?:  
The addresses are saved localy on the users device by using IndexedDB.
I used the IDB backage that makes it easier to use.

Addresses are listed accoring to usage.
...
The app additionaly displays Tolls on the route as it has a list of all the toll gates in south Africa including their coordinates and prices.
...
If a toll stretches over a wide double road highway each side has a seperate cooridante as well.
...
So I opted for very tight tolerances and mutliple coordinates to eliminate errors.
...
How does it work?:  
The route is seperated into legs, the distance and duration of each leg is used to display inter address statistics.
...
I did not test to see if there is a performance improvement, so this might be a case of premature optimisation.
...
How does it work?:  
The app saves the route on MongoDB, giving it a unique ID that can be used to retreive it.
...
It looks similar to the former steps but here the route is not ment to be edited, 
...
A autorize feature will be added later.
...
It lookes similar to the when it was being edited.
Clicking on the "road sign" buttons opens google map with the corisponding address loaded as the destination.
